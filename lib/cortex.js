...
class Cortex extends EventEmitter {
  constructor(options = {}) {
    ...
    this.ready = new Promise(
      ...
      .then(() => {this._log("ws: Socket opened")}, err => {console.log(err)})
      .then(() => {return this.call("inspectApi")}, err => {console.log(err)})
      .then(methods => {
    ...
    const result = this.getUserLogin()
      .then(users => {
        if (users[0] && users[0] !== username) {
          ...
          return this.logout({ username: users[0] }).then(() => [], err => {console.log(err)});
        }
        if (username) {
